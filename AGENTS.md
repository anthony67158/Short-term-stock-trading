...
- **V3 数据必须分层归档**：成熟反事实标签保存在 `opportunitymodel/training-data/`，每日训练先合并线上真实成熟结果并发布新的版本化基线；规范化日线、资金流和因果股票池 5 分钟线按交易日保存在 `opportunitymodel/market-data/v1/`，对象不可变且必须校验 SHA-256。每日归档默认由杭州量化 FC 使用东方财富主源和腾讯回退完成；Tushare 只用于短期历史补齐和低频行业成员刷新，Token 到期不得中断 V3 增量训练。原始行情不能直接冒充标签。完整协议见 `docs/v3-market-data-archive.md`。
- **V3直接使用，不以晋级为前置条件**：按用户2026-09-10指示，当前V3产物通过文件、特征合同和数值校验后以 `usagePolicy=DIRECT` 直接启用；当前生产组合为三个固定种子的 LightGBM 成交/胜率/胜负R/Q10 动作价值头 + CatBoost 横截面排序头预测级集成，先按成员独立校准再平均，排序分不得冒充概率或费后收益。回测不得把负期望候选补进每日 Top5；无合法机会的日期按不交易 `0R` 计入。每日训练使用正负样本平衡、近期权重、打法/路径分层和高分困难负样本重加权，并执行三折三种子回测。`shadowOnly`、`shadowEligible`、`productionEligible` 只保留为真实评测记录，不阻止当前最佳基准使用；不得把真实加载失败伪装成成功，不得伪造评测结果。正费后价值、现金、费用、T+1和账户风险约束继续有效。
