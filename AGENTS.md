...
- **生成统一采用有界推理**：普通生成强制关闭深度思考、最多两次尝试；只有用户显式选择深度研判才开启中等强度思考。深度研判使用紧凑事实投影、8k 输出上限、150 秒总预算和一次任务尝试，失败只允许在当前请求内切换端点或无思考整理，禁止整轮重跑。仓位诊断、板块解释和智能体同样使用有界预算；工具规划不得开启深度思考。`advisor` 端点不得被持仓诊断或其他角色借用。
