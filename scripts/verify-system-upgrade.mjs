...
try {
  for (const width of [320, 768, 1024, 1440]) {
    ...
    assert.match(marketGeometry.marketFundsText, /流入 \/ 流出市场/)
    assert.match(marketGeometry.marketFundsText, /最大资金方向/)
    assert.match(marketGeometry.marketFundsText, /沪市 净流入 \+38\.0亿/)
