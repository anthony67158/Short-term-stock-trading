...
当前生产决策架构是 `docs/multi-task-trading-engine-redesign.md` 定义的模块化多任务引擎：
...
- 账本硬止损直接执行；模型驱动的普通减仓/退出先进入约60秒
  `holding-exit` 复核，使用最新价格、分时和资金重新运行系统决策。反弹和继续下跌
...
机会模型使用 `opportunity-score-feature.v6`（136 维，包含五日主力/小单
资金序列、数据可用性标识，以及五类日线形态原语和连续强度分）。生产组合固定为三个种子的
...
不得成为决策模型每日训练依赖。夜间定时任务必须取得最新完整交易日归档并读回
校验 OSS 内容；端点失败、分片缺失或覆盖不足必须阻断决策模型训练。盘中手动
