...
class TrainOpportunityScoreTest(unittest.TestCase):
    ...
    def test_training_weights_emphasize_recent_market_regimes(self):
        value = dataset(samples=20, dates_count=20)
        weights = _training_weights(
            {
                **value,
                "playbook_ids": np.asarray(["A"] * 20),
                "routes": np.asarray(["IMMEDIATE"] * 20),
            },
            np.arange(20),
        )
    ...
    def test_ready_dataset_trains_hurdle_q10_and_ranker_heads(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch(
                    "decision_engine.training.trainer._fit_lgb_classifier",
                    side_effect=classifier,
                ),
                patch(
                    "decision_engine.training.trainer._fit_lgb_regressor",
                    return_value=FakeRegressor(),
                ),
                patch(
                    "decision_engine.training.trainer._fit_lgb_quantile_regressor",
                    return_value=FakeRegressor(),
                ),
                patch(
                    "decision_engine.training.trainer._fit_catboost_ranker",
            ...
            self.assertEqual(
                meta["calibration"]["pWinGivenFill"]["method"],
    ...
    def test_rejected_evaluation_still_exports_real_weights(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            np.savez_compressed(path, **value)
            with (
                patch("decision_engine.training.trainer._fit_lgb_classifier", return_value=FakeClassifier()),
                patch("decision_engine.training.trainer._fit_lgb_regressor", return_value=FakeRegressor()),
                patch("decision_engine.training.trainer._fit_lgb_quantile_regressor", return_value=FakeRegressor()),
                patch("decision_engine.training.trainer._fit_catboost_ranker", return_value=(
                    FakeRanker(),
                    {"zeroOrUnfilled": 1, "positiveMedian": 1.0, "positiveHigh": 1.0},
                )),
                patch("decision_engine.training.trainer.shadow_gate", return_value={
