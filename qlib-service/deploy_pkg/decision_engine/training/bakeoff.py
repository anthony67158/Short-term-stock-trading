"""Run an offline, same-split decision-model bake-off."""
...
from .backtest_dataset import build_poc_dataset, interval_expanding_folds


POC_SCHEMA_VERSION = "decision-model-bakeoff.v1"
...
def run_family_fold(family, dataset, fold):
    ...
    win_calibrator = fit_stratified_probability_calibrator(
        dataset["y_win"][filled_calibration].astype(np.int8),
        _probability(win_model, X_filled_calibration),
        dataset["playbook_ids"][filled_calibration],
        dataset["routes"][filled_calibration],
        dates=dataset["dates"][filled_calibration],
    )
    global_win_calibrator = fit_probability_calibrator(
        dataset["y_win"][filled_calibration].astype(np.int8),
        _probability(win_model, X_filled_calibration),
    )
    ...
    raw_win_validation = _probability(win_model, X_validation)
    win_probability_all = apply_probability_calibrator(
        raw_win_validation,
        win_calibrator,
        playbook_ids=dataset["playbook_ids"][validation],
        routes=dataset["routes"][validation],
    )
    global_win_probability_all = apply_probability_calibrator(
        raw_win_validation,
        global_win_calibrator,
    )
    ...
    raw_win_calibration = _probability(win_model, X_calibration)
    ...
    return {
        ...
        "_selection": {
            ...
            "rawPWin": raw_win_validation.astype(float).tolist(),
            "calibratedPWin":
                win_probability_all.astype(float).tolist(),
            "globalCalibratedPWin":
                global_win_probability_all.astype(float).tolist(),
            "pWinCalibration": win_calibrator,
            ...
            "calibrationRawPWin":
                raw_win_calibration.astype(float).tolist(),
            "calibrationExpectedNetR": compose_expected_net_r(
                apply_probability_calibrator(
                    raw_win_calibration,
                    win_calibrator,
                    playbook_ids=dataset["playbook_ids"][calibration],
                    routes=dataset["routes"][calibration],
                ),
    ...
    for action_fold, ranker_fold in zip(lightgbm_folds, catboost_folds):
        ...
        for weight in (0.0, 0.25, 0.5, 0.75, 1.0):
            ...
            blend_trials.append({
                ...
                "netRLowerBound":
                    metrics["top5"]["netRLowerBound"],
                "maxDrawdownRAt5":
                    metrics["top5"]["max_drawdown_r_at_5"],
