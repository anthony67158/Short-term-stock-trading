...
def _member_predictions(member, X):
...
def select_ensemble_blend(
    ...
    eligible = [
        value
        for value in trials
        if value["positiveExpectedCoverage"] >= 0.02
    ] or trials
    selected = max(
        eligible,
        key=lambda value: (
...
def _fit_member(data, development, calibration, seed):
    ...
    win_calibration = fit_probability_calibrator(
...
def train_seed_ensemble(
    ...
    *,
    ...
    report = train_opportunity_score(dataset_path, output_directory)
    ...
    predictions = [
        _member_predictions(member, data["X"][holdout])
