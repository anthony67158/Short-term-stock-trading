"""Select and validate a component-level V3 production release."""
...
from opportunity_model import (
    ...
    _ensemble_prediction_arrays,
    _load_release,
    validate_opportunity_metadata,
)
from time_splits import three_way_purged_split
from train_opportunity_score import load_opportunity_dataset
...
def compose_release(
    ...
    for metadata in (champion_metadata, challenger_metadata):
        ...
        if (
            ...
            raise ValueError("成员级晋级只支持V3三种子集成")
    if (
...
def _load_bundle(directory):
    ...
    if (
        ...
        raise ValueError("当前生产V3不是可选择晋级的三种子集成")
    with open(artifact_path, encoding="utf-8") as handle:
        artifact = json.load(handle)
    models, loaded_metadata = _load_release(
...
def main():
    parser = argparse.ArgumentParser(
        description="按组成部分比较并选择V3生产发布组合",
    )
