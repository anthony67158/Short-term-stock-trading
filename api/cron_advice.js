...
export function progressPatchForEvent(event, data) {
  ...
  if (event === 'agent') return { agentPreview: data };
...
async function runJobGen(
  ...
  const result = await runDecision({
    ...
    signal,
    reviewEvent,
    onProgress: (phase, stage, detail = null) => onProgress?.({
      phase, stage,
      ...(detail?.agentPreview
        ? { agentPreview: detail.agentPreview }
        : {}),
  ...
  cacheItem.generationMetrics = {
    ...
    profile: 'DECISION',
    durationMs: Date.now() - startedAt,
    mainLlmCalls: Number(result.meta?.llmCalls) || 0,
  };
  return {
    cacheItem,
    logEntry: {
    ...
    try {
      ...
      return res.end(JSON.stringify({
        ...
        alert: currentAlert ? {
          ...
          watchingAt: currentAlert.watchingAt,
          watchingPrice: currentAlert.watchingPrice,
          watchingMsg: currentAlert.watchingMsg,
          monitoringUntilAt: currentAlert.monitoringUntilAt,
