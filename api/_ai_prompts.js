...
【★盈亏比与目标价现实性铁律(所有含目标价/止损价的建议必须自检)】目标价 target 不是越高越好——它要落在"3个交易日内、约1~2倍ATR真实波幅够得着"的范围内(参考 tech.atr、量化 targetHigh、近期压力位)，虚高的目标价会让回测命中率崩塌、也让用户追不上。止损价则要给在最近关键支撑下方一档、且亏损幅度≤8%。给出结论前必须心算:(目标价-买入价)/(买入价-止损价) 的盈亏比是否≥2:1,达不到就别喊"立即买入/加仓",改"回调再买/小仓试错/观望"。共振分<2 或 盈亏比<1.8:1 一律不得给激进结论。${(payload.account && payload.account.goal) ? `
...
【★手数铁律·做T卖出腿绝对不能违反】用户当前实际持仓 holdQty=${payload.holdQty ?? '—'}手(已含未结算做T净腿)。反T(先卖后买)或任何先卖的腿,卖出手数 suggestQty【绝对不能超过 ${payload.holdQty ?? 'holdQty'} 手】——他手里只有这么多底仓,卖不出比持仓更多的量;若 holdQty 为 0 或空,则不能做反T(没有底仓可先卖),只能做正T(先买后卖)。正T的买入手数则受可用资金约束。自检:suggestQty ≤ ${payload.holdQty ?? 'holdQty'}(反T时)吗?
...
【★手数铁律·绝对不能违反(基于真实持仓)】${payload.hold ? `用户当前实际持仓 holdQty=${payload.holdQty ?? '—'}手,这是他真实交易记录算出来的实时持仓(已含未结算做T净腿)。复盘里【只能基于这个真实手数】给建议——任何"持有X手/减仓X手/清仓/止损减半"里提到的手数【绝对不能超过 ${payload.holdQty ?? 'holdQty'} 手】,更不能凭空捏造一个手数(如实持3手却说"持有4手")。"清仓"就是全部 ${payload.holdQty ?? 'holdQty'} 手;"减仓/减半"只能在 1~${payload.holdQty ? Math.max(1, payload.holdQty - 1) : 'holdQty-1'} 手之间;nextAction/headline/keyLevel 里凡提到持仓手数,都必须等于 ${payload.holdQty ?? 'holdQty'} 手。` : '用户当前未持仓,不要给"持有X手/减仓"这类建议,只做关注级研判。'}
