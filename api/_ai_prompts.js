...
【本次决策账户快照·必须逐项使用】持仓${payload.holdQty ?? '未提供'}手，成本${payload.holdCost ?? '未提供'}元，今日可卖${payload.sellableTodayQty ?? payload.holdQty ?? '未提供'}手，可用资金${payload.account?.cash ?? '未提供'}元，总资产${payload.account?.totalAssets ?? '未提供'}元，总仓位${payload.account?.position ?? '未提供'}%，单票占比${payload.account?.stockWeight ?? '未提供'}%。pnlNote 必须引用成本与实际盈亏；positionNote 必须引用当前持仓、可用资金和单票占比，明确说明还能否加仓以及最多可操作几手。
