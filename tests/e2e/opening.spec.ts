...
test("期初持仓不扣现金、刷新、卖出FIFO与独立核对", async ({ browser, playwright }) => {
  ...
  try {
    ...
    await page.getByRole("button", { name: "核对账本", exact: true }).click();
    await expect(page.getByText("账本核对一致", { exact: true })).toBeVisible();
    await page.getByRole("button", { name: "录入股份转入", exact: true }).click();
    await page.getByLabel("转入证券代码").fill("000001");
    await page.getByLabel("转入股数", { exact: true }).fill("13");
    await page.getByLabel("转入剩余总成本（元）").fill("130.01");
    await page.getByLabel("原股份取得日（上海日期）").fill(local(8).slice(0, 10));
    await page.getByLabel("转入到账时间（本设备时区）").fill(local());
    await page.getByLabel("转托管凭据编号").fill("synthetic-transfer-1");
    await page.getByLabel("转托管凭据说明").fill("合成转托管批次");
    await page.getByRole("button", { name: "保存股份转入" }).click();
    await expect(page.getByText("股份转入已保存，现金未变动。")).toBeVisible();
    await page.reload();
    await expect(page.locator(".balance-value")).toHaveText("138.87");
