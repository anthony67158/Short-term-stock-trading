...
test('条件试仓到价后只确认入场时机并要求输出具体价格', () => {
  ...
  assert.match(prompt, /观察60秒/)
  assert.match(prompt, /2分钟内裁决/)
  assert.match(prompt, /核对原计划/)
  assert.match(prompt, /立即买入”“维持观望”“放弃买入/)
  assert.match(prompt, /不得生成新观察价/)
  assert.match(prompt, /只据窗口事实/)
  assert.match(prompt, /之后量价资金转强才重评/)
...
test('观望买入建议区分观察锚与买入价并比较两条入场路径', () => {
  ...
  for (const prompt of [normalPrompt, fastPrompt]) {
    ...
    assert.match(prompt, /invalidation必须写成可观测条件/)
    assert.match(prompt, /禁止断言未来“不能\/不会重新站回”/)
    assert.match(prompt, /之后重新站回.*作为新事件重新评估/)
  }
})
