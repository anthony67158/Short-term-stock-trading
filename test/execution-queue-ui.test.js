...
test('个股建议只保留一个主结论并把扩展信息收进详情', () => {
  const presentation = read('src/components/DecisionSummary.jsx')
  const stockDetail = read('src/components/StockDetail.jsx')
  const generation = read('src/components/AdviceGenerationStatus.jsx')
  const css = read('src/styles/precision.css')

  assert.match(presentation, /decision-headline/)
  assert.match(presentation, /何时操作/)
  assert.match(presentation, /参考价格/)
  assert.match(presentation, /风险边界/)
  assert.match(presentation, /decisionPrices\(advice\)/)
  assert.match(presentation, /aria-label="复核与执行价位"/)
  assert.match(presentation, /fmtRaw\(item\.value\)/)
  assert.match(presentation, /决策依据/)
  ...
  assert.doesNotMatch(stockDetail, /loadQuant\(true\)/)
  assert.match(stockDetail, /更新决策/)
  assert.match(stockDetail, /查看依据/)
  assert.doesNotMatch(stockDetail, /className="quant-cta-actions"/)
  assert.match(stockDetail, /系统决策已保存/)
  assert.doesNotMatch(stockDetail, /className="sk-hint"/)
  assert.match(generation, /advice-generation-flow/)
  assert.match(generation, /label: '决策评估'/)
  assert.match(generation, /generation-flow-steps/)
  assert.match(generation, /下方是上次结论，不是本轮结果/)
  assert.match(css, /\.decision-price-strip\s*{[^}]*display:\s*flex[^}]*overflow-x:\s*auto/s)
  assert.match(css, /\.decision-price-strip > div \+ div\s*{[^}]*border-inline-start:/s)
  assert.match(
    css,
...
test('个股详情底栏直接呈现三项命令且不重复展示功能说明', () => {
  ...
  assert.doesNotMatch(stockDetail, /adviceModeGuidance/)
  assert.match(stockDetail, /footbar-evidence/)
  assert.match(stockDetail, /decisionAnchorRef\.current\?\.scrollIntoView/)
