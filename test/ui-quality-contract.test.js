...
test('个股生成区实时展示Agent阶段与可公开结构化内容', () => {
  assert.match(generationStatus, /\{ key: 'agent', label: 'Agent研判' \}/)
  assert.match(generationStatus, /aria-label="Agent生成过程与内容"/)
  assert.match(generationStatus, /Agent生成实况/)
  assert.match(generationStatus, /现在怎么做/)
  assert.match(generationStatus, /为什么这样做/)
  assert.match(generationStatus, /什么情况先别做/)
  assert.match(generationStatus, /到价后检查/)
  assert.match(generationStatus, /什么时候作废/)
  assert.match(precision, /\.generation-agent-output\s*{[^}]*max-height:/s)
  assert.match(
    precision,
    /@media \(max-width:\s*720px\)[\s\S]*?\.generation-agent-slots\s*{[^}]*grid-template-columns:\s*1fr/s,
  )
})
...
test('所有按钮受父容器约束且持仓只保留一个推荐主动作', () => {
  ...
  assert.match(
    precision,
    /\.hold-swipe-wrap:has\(\.holding-more-actions\[open\]\),[\s\S]*?overflow:\s*visible/,
  )
  assert.match(
    precision,
    /\.plan-section-hold:has\(\.holding-more-actions\[open\]\)\s*{[^}]*z-index:\s*200/s,
  )
  assert.match(
    planTab,
...
test('持仓与自选卡内部控件在明暗主题都有实体表面和边界', () => {
  ...
  assert.match(
    precision,
    /\.plan-cand \.pc-actions\.deferred \.review-action\s*{[^}]*border-color:\s*var\(--color-trade-control-border\)[^}]*background:\s*var\(--color-trade-control\)[^}]*color:\s*var\(--color-ink-2\)/s,
  )
  assert.match(
    tokens,
