...
test('机会候选同时展示入场仓位和完整退出计划', () => {
  ...
  assert.match(opportunityUi, /Agent优先级/)
  assert.match(opportunityUi, /买入理由/)
  assert.match(opportunityUi, /暂停条件/)
  assert.match(opportunityUi, /失效条件/)
  assert.match(opportunityUi, /MARKET_FUND_FLOW/)
  assert.match(opportunityUi, /SEARCH_RESULT_UNVERIFIED/)
...
test('自适应作战台提示上一模型版本结果需要重新扫描', () => {
  ...
  const snapshot = {
    sourceStatus: {
      ...
      preCatalyst: { status: 'fresh' },
    },
  }
  assert.equal(
    opportunityRadarHasStaleModelSource(snapshot, 'intraday'),
    true,
  )
  assert.equal(
...
test('统一客户端使用聚合接口并保留独立来源刷新', () => {
  ...
  assert.match(client, /AGENT_RETIRED/)
...
test('预催化来源并入提前布局且展示独立运行状态', () => {
  ...
  assert.match(content, /agentIntraday: '盘中Agent'/)
  assert.match(content, /agentNext: '次日Agent'/)
  assert.match(content, /'formulaIntraday'[\s\S]*'preCatalyst'[\s\S]*'tail'[\s\S]*'agentIntraday'/)
...
test('盘中刷新并行启动板块和盘中公式且局部失败可返回', async () => {
  ...
  const pending = refreshOpportunityRadar({
    ...
    invalidateAgentSelection: async () => { invalidated = true },
...
test('异步来源返回运行中时等待权威DONE快照后再进入Agent', async () => {
  let reads = 0
  const result = await waitForOpportunitySources({
    sources: ['tail'],
    now: (() => {
      let value = 0
      return () => value += 100
    ...
    load: async () => {
      ...
      return reads === 1
        ? {
            sourceStatus: { tail: { status: 'running' } },
            tasks: { tail: { status: 'RUNNING' } },
          }
        : {
            sourceStatus: { tail: { status: 'fresh' } },
            tasks: { tail: { status: 'DONE' } },
          }
    },
  })
  ...
  await assert.rejects(
    waitForOpportunitySources({
      ...
      load: async () => ({
        ...
        tasks: { sector: { latest: { status: 'failed' } } },
      }),
    }),
    /sector更新失败/,
  )
...
test('板块active-generation返回也必须等待DONE后再运行Agent', async () => {
  const calls = []
  await refreshOpportunityRadar({
    lane: 'intraday',
    snapshot: { phase: 'INTRADAY', sourceStatus: {} },
    invalidateAgentSelection: async () => calls.push('invalidate'),
    runSector: async () => ({
      ok: true,
      skipped: true,
      reason: 'active-generation',
    }),
    runFormula: async () => ({ ok: true }),
    runPreCatalystScan: async () => ({ ok: true }),
    waitForSources: async ({ sources }) => {
      calls.push(`wait:${sources.join(',')}`)
    },
    runAgentSelection: async () => calls.push('agent'),
    load: async () => ({ ok: true, lanes: {} }),
  })
...
test('盘前次日关注只读昨晚计划且不启动生成任务', async () => {
  ...
  await refreshOpportunityRadar({
    ...
    invalidateAgentSelection: async () => {},
    runAgentSelection: async (lane) => calls.push(['agent', lane]),
