...
test('每日学习在FC收盘结算并由Actions次日训练challenger', () => {
  const config = read('s.yaml')
  const server = read('server.js')
  const workflow = read('.github/workflows/daily-retrain.yml')

  assert.ok(config.includes('- triggerName: learning-settlement-timer'))
  assert.ok(config.includes(
    'cronExpression: "CRON_TZ=Asia/Shanghai 0 20 17 * * 1-5"',
  ))
  assert.match(server, /learningTimerBody\(/)
  assert.match(server, /learningBody[\s\S]*'cron_learning'/)
  assert.match(workflow, /cron: "15 17 \* \* 0-5"/)
