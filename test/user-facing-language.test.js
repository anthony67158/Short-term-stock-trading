...
test('Agent旧内容压缩为两句并隐藏内部字段名', () => {
  const result = conciseUserFacingText(
    '当前应等待55.44元确认。pFill为0.62，q10R为-1.5，expectedShortfall10为-2.3。第三句不应展示。',
    { maxLength: 120 },
  )

  assert.match(result, /预计成交概率/)
  assert.match(result, /较差情形收益/)
  assert.match(result, /尾部平均亏损/)
  assert.doesNotMatch(result, /pFill|q10R|expectedShortfall10|第三句/)
})
