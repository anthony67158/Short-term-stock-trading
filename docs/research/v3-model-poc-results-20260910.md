...
进一步复核发现，旧回测虽然把 `expectedNetR <= 0` 的候选分数降到最低，
但每日排序仍强制补满 Top5，导致系统明确不交易的候选也被计入收益。修复后：

- 无合法候选的日期按不交易 `0R` 计入，不跳过该日期；
- 每日只统计实际通过正期望门槛的 0–5 只股票；
- 单种子组合最差下界由 `-0.0508R` 改善至 `-0.0417R`；
- 三个种子的逐候选预测先独立校准再平均，只输出一套 Top5；
- 预测级集成 Top5 均值为 `+0.3361R`，95% bootstrap 下界为
  `+0.1047R`；
- 80 条候选本地批量推理 P95 约 `14ms`。

该结果证明原负下界主要由“强制补位”和单一种子方差共同造成。但最新独立窗口
均值仍为 `-0.0570R`、窗口下界为 `-0.5253R`，因此集成模型继续保留
`productionEligible=false`，不能宣称已经获得跨所有市场阶段的稳定盈利能力。
