...
- [x] 1.5 Task 2 封存数据后写入真实 hash，生成最终 `experiment.json`；
- [x] 2.0 审计全市场收益样本与分钟执行标签覆盖，确认两类监督不可混写；
- [x] 2.1 构建 90 日点时序列；
- [x] 2.2 构建五日费后收益、方向、成交与成交比例标签；
- [x] 2.3 完成 purge/embargo、抽样权重和全市场测试对账；
- [x] 3.1 训练 CatBoost MultiQuantile 基线；
- [x] 3.2 训练 XGBoost quantile 基线；
- [x] 3.3 训练 LightGBM quantile 基线；
- [x] 4.1 以零付费本机 MPS/CPU 运行 TTM、TimesFM、Chronos 固定筛选；
- [x] 4.2 按预注册分数与灾难门禁冻结判定：三者均失败，不强选主候选；
- [x] 4.3 按用户停止决定删除本机训练数据、模型产物和专用缓存；
- [ ] 5.1 实现轻量分布头和 checkpoint 恢复（已取消，不执行）；
- [ ] 5.2 完成单折单种子 smoke test（已取消，不执行）；
- [ ] 6.1 运行前四折三种子并冻结配置（已取消，不执行）；
- [ ] 6.2 一次性运行第五确认折（已取消，不执行）；
- [ ] 7.1 完成概率、区间、拒答和执行校准（已取消，不执行）；
- [ ] 7.2 完成容量、持仓及 Agent 联合消融（已取消，不执行）；
- [ ] 8.1 生成模型包、成本报告和发布判定（已取消，不执行）。
...
- [x] 配置包含所有输入 hash、权重 revision 和预算上限；
- [x] 第 5 折时间范围已冻结但结果未读取；
- [x] 活动生产指针未改变。

**验证：**

- [x] 配置 schema 测试通过；
- [x] 同配置重复生成相同 experiment ID。
...
- [x] 2.1.1 绑定 sealed ranking、market 与 execution-label 三个上游 hash；
- [x] 2.1.2 以虚拟数据集方式流式读取 90 日 OHLCV 与复权因子，不复制 10M 窗口；
- [x] 2.1.3 校验序列严格截止决策日，且 `featureAvailableAt` 与 ranking 一致；
- [x] 2.2.1 定义 `REFERENCE_FULL_FILL_FEE_ADJUSTED_5D` 全市场参考标签；
- [x] 2.2.2 按 10 万元完整成交口径扣除买卖费用及卖出滑点；
- [x] 2.2.3 分离分钟执行监督，未覆盖与未成交分别表示为缺失和零成交；
- [x] 2.2.4 封存 10,223,999 个参考样本及 19,486 个执行覆盖样本；
- [x] 2.3.1 冻结五折日期边界、两段 63 日校准、5 日 purge 与 5 日 embargo；
- [x] 2.3.2 封存全市场逐日总市值与流通市值，完成全部 2,536 个日期；
- [x] 2.3.3 实现按日期、板块、市值、流动性和收益方向的确定性抽样；
- [x] 2.3.4 生成每折 1,500,000 窗口的抽样索引及逆概率权重；
- [x] 2.3.5 对账训练抽样、全量校准、全量测试和 manifest hash；

**验收：**

- [x] purge/embargo 和同日分组正确；
- [x] 测试与回测覆盖全量股票；
- [x] 未成交样本不被错误标为零收益。
...
- 虚拟基础数据集：10,223,999 个参考样本、2,536 个交易日、5,760 个标的；
- 分钟执行覆盖：19,486 个 `reference_100k` 样本，其中成交 17,161、未成交
  2,325；无分钟覆盖仍为缺失，不写成零收益；
- 五折训练索引：每折 1,500,000 条；测试集分别为 1,104,625、1,220,602、
  1,310,884、1,318,408、1,335,778 条全量样本；
- 数据 hash：基础库 `6f8a8323…e3a8fa3`、市值库 `41dc40b6…1ddc3a`、
  抽样库 `a9ac39af…73b382`、切分清单 `b5711163…845f13`；
- 实验 ID：`foundation-return-100rmb-v1-9ded11c2a2508c69`；
- 累计付费：`0.00` 元；状态：`UNAVAILABLE`；生产指针未修改。
...
- [x] 3.1.1 锁定 CatBoost 1.2.10、XGBoost 3.4.1、LightGBM 4.7.0；
- [x] 3.1.2 复现 10 万元完整成交费后标签，逐笔对齐 Decimal 权威实现；
- [x] 3.1.3 训练方向发生率与七分位历史分布基线；
- [x] 3.1.4 训练 CatBoost 单模型 `MultiQuantile` 与方向分类器；
- [x] 3.2.1 训练 XGBoost `reg:quantileerror` 多输出分位与方向分类器；
- [x] 3.3.1 训练七个 LightGBM quantile 回归器与方向分类器；
- [x] 3.3.2 统一计算 Brier、log loss、pinball、WIS、80% coverage、RankIC
  与 Top-10 费后收益；
- [x] 3.4.1 完成 fold 1 小样本四 family 端到端 smoke；
- [x] 3.4.2 冻结 120 trees、depth 6、learning rate 0.05 和 seed 20260917；
- [x] 3.5.1 运行前四开发折并封存 16 份 receipt；
- [x] 3.5.2 审计 4,954,519 条开发 OOF，重复键与折间重叠均为 0；
- [x] 3.5.3 冻结 raw baseline 无校准策略及 Task 7 校准候选集；
- [x] 3.6.1 一次性运行第五确认折，不改参数、不重排候选；
- [x] 3.6.2 封存每个 family 6,290,297 条五折 OOF 与 20 份 receipt；

**验收：**

- [x] 三类 GBDT 至少各有一份完整 OOF；
- [x] 指标和费用场景可复跑；
- [x] 基线版本不可被候选训练覆盖。
...
- 正式产物位于仓库外 `foundation-return-100rmb-v1/baselines-v1`，协议 hash 为
  `9933b815…f732e3`，最终 manifest 的 OOF audit hash 为
  `58814e6e…4325f0`；
- 每个 family 覆盖五折 1,260 个测试日、6,290,297 个全市场样本；唯一键重复 0，
  折间重叠 0，跨 family 标签、证券、日期和样本权重完全一致；
- 五折聚合 WIS：LightGBM `0.029253`、CatBoost `0.029264`、XGBoost
  `0.029451`、历史分布 `0.030598`；原始 80% coverage 分别为 `77.14%`、
  `77.71%`、`79.02%`、`77.96%`，尚未执行 Task 7 校准；
- 五折 Top-10 费后增量仅 LightGBM 为正（`+0.1922%`）；CatBoost、XGBoost
  与历史分布分别为 `-0.0810%`、`-0.0562%`、`-0.0680%`。确认折 LightGBM
  增量为 `-0.0332%`，因此不能据开发折聚合结果宣称通过交易门禁；
- 12 项聚焦测试及 Ruff 通过；累计付费 `0.00` 元，状态仍为 `UNAVAILABLE`，
  生产指针和交易事实均未修改。
...
- [x] 4.1.1 锁定三模型官方 revision、Python 3.12 隔离环境和完整依赖；
- [x] 4.1.2 验证 TTM 日频 token、TimesFM 原生九分位和 Chronos 原生七分位 API；
- [x] 4.1.3 废弃使用成熟 `r_net_5d` 历史作为输入的 teachers-v1/v2/v3 预实验；
- [x] 4.2.1 生成 teachers-v4：90 日点时 `adjustedClose`、参考收盘价与执行日期；
- [x] 4.2.2 固定预测未来第 5 个交易步，目标仍为 next-open 至 fifth-close 费后收益；
- [x] 4.2.3 每个日期确定性选择 50 只，不足完整 90 日历史的候选按 hash 顺序跳过；
- [x] 4.2.4 对账两个开发折各 63 个校准日、252 个测试日和 15,750 行；
- [x] 4.3.1 完成 TTM MPS 推理，31,500 行耗时 24.33 秒；
- [x] 4.3.2 完成 TimesFM CPU 推理，31,500 行耗时 326.66 秒；
- [x] 4.3.3 完成 Chronos CPU 推理，31,500 行耗时 192.18 秒；
- [x] 4.3.4 封存三模型 raw forecast、receipt、两折预测和 evaluation hash；
- [x] 4.4.1 将第五日预测价换算为 close-to-close 费后代理，只用校准窗吸收入口 gap；
- [x] 4.4.2 在相同 `(date, instrument)` 键上抽取并同法校准三类 GBDT；
- [x] 4.4.3 按固定权重计算 WIS、Brier、RankIC 和费后收益 skill；
- [x] 4.4.4 三候选均触发灾难门禁，冻结 `selectedModel=null` 和停止结论；

**验收：**

- [x] 免费计算未产生付费；
- [x] 三个候选均记录成功终态，异常路径可封存明确失败原因；
- [x] 按预注册分数完成判定；因无合格候选，不违反门禁强行选择。

**验证：**

- [x] 本机免费运行产物 hash 已封存并可重复读取；
- [x] 候选选择仅使用第 1、2 开发折，未读取第 5 折。
...
- teachers-v4 共 31,500 行，数据 SHA-256 为 `d4e90229…40cb3d`；
- exact-key 同校准基线中，最佳 WIS/Brier 为 XGBoost，最佳 RankIC 为 CatBoost，
  最佳 Top-10 费后增量为 LightGBM；
- TTM、TimesFM、Chronos 综合分分别为 `-0.5506`、`-0.6472`、`-0.8374`；
  三者两折平均 Top-10 费后增量分别为 `-0.0942%`、`-0.1565%`、`-0.2255%`；
- TTM 虽为相对最好候选，但 fold 1 WIS 劣化 `10.14%`、Brier 劣化 `7.03%`，
  且两折费后增量均负；TimesFM 两折费后增量均负；Chronos 两折 RankIC 均负；
- 最终 `teacher-screening-v6` SHA-256 为 `7be756a2…1bca45`，
  `selectionStatus=STOPPED_ALL_CANDIDATES_FAILED`、`selectedModel=null`；
- 累计付费仍为 `0.00` 元；第 5 折、Task 5-8、生产指针和交易事实均未触碰。
- 用户确认预测模型无继续价值后，已删除本机可重建训练数据、OOF、模型产物、
  教师运行环境和 Hugging Face 三模型缓存；实验 hash 仅作为历史记录保留在文档。
- 清理前磁盘可用 `21GiB`，清理后为 `137GiB`，实际释放约 `116GiB`。
- 保留数据库备份、迁移记录、活动 `SHADOW` 发布指针、联合候选注册表和每日运行记录；
  生产 `https://www.tedixtf.cn/__health` 清理后仍返回 `ok`。

**停止结论：** Task 5-8 依赖唯一合格主候选。当前实验在 Task 4 触发停止条件，
不得继续轻量微调、五折三种子或发布；若继续研究，必须登记新实验版本和新假设，
不能沿用本实验结果绕过门禁。
