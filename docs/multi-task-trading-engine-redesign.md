...
- 现役三种子树模型由新 registry、共享状态编码、路由与任务头加载，服务端只
  消费 `decision-score` 和统一动作价值；旧单体主链已删除。
- 持仓历史尚无可训练的动作状态样本，生产明确使用
  `position-head.adapter-v1`，不得把适配值宣称为独立持仓专家模型。
...
- Node 与 Python 模块化运行时、训练发布入口和部署包已完成迁移；
- 旧 V3 单体推理、训练入口和用户可见命名已删除；
- 同花顺扶摇已接入报价、日线与市场归档，真实全市场覆盖率验证为 99.623%；
- Node 2210/2210、Python 277/277、Harness 53/53、构建与 FC 打包通过；
- 量化 FC、主 FC、Vercel 与备案域名已部署，测试账号浏览器链路已验收。
...
Data-gated follow-ups:

- 生产历史共有 73,547 条成熟路径、37,597 条完整成交和 127 个交易日，但
  `positionMode` 覆盖为 0，尚不能训练独立 ADD/REDUCE/EXIT 头；
- 新 `decisionState`、`actionValues` 与真实成交需继续积累后，再启用持仓
  反事实标签和账户级顺序回放；
- PyTorch MMoE/PLE 仍是挑战者研究，不进入生产，除非同切分门禁优于当前
  树模型基线。
