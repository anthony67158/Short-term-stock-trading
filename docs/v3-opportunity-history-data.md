...
- 初始历史样本使用 `opportunity-score-feature.v3`。
...
第二轮重放已升级为 `opportunity-score-feature.v4`：

- 72,847 个成熟候选、37,491 个完整成交结果、120 个信号日；
- 104 个首次决策特征；
- 历史行业阶段按成员生效区间重建；
- 四路召回分位、召回来源和探索样本标记完整入库；
- 触发后复核特征使用独立 `opportunity-review-feature.v1`，不进入首次决策。

V4 成熟基线已发布到：

```text
opportunitymodel/training-data/runs/
  1789034554302-60de980d22fa7be6.json.gz
```
...
该 Token 是短期历史补齐凭证，不属于长期训练依赖。初始历史和行业成员归档
完成后，每日 V3 数据改由东方财富全市场快照、东方财富 5 分钟历史接口和腾讯
5 分钟回退采集，并在杭州量化 FC 内写入 OSS。GitHub Actions 只触发该鉴权
端点并训练 OSS 中已成熟的数据；Token 到期不会中断 V3 增量训练。
...
→ 三路径成交与退出结算
→ V3数据集
→ LightGBM稳健动作价值 + CatBoost排序训练
→ 三折三种子walk-forward回测
→ 更新DIRECT基准并如实记录评测状态
→ 通过全部硬门槛时追加productionEligible记录
```

按用户 2026-09-10 的最新选择，当前样本上效果和潜力最佳的
