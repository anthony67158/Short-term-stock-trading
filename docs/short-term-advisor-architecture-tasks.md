...
- [x] 部分共振机会进入最多总资产 5% 的人工试仓，正式建仓仍要求全部确认。
- [x] 高于现价的候选买入价降为突破观察价，不得作为低吸执行价。
- [x] 非连续竞价时段只生成盘中复核计划，不展示到价或发送即时提醒。
- [x] 休市时保留下午/次日的条件买入或5%试仓预案，不把后续逻辑压扁为观望。
- [x] 流动性说明展示真实成交额、数据来源及 1 亿元短线门槛。
- [x] 快速与深度军师都读取 MA、MACD、KDJ、BOLL、RSI、ATR 和量比。
- [x] 盘中技术与多维信号共振时优先评估当前价试仓，不机械等待更低回踩。
- [x] 到达观察价后自动核验分时承接、VWAP、动量和量能，无需手动深度生成。
