...
## Phase 6：Alpha158 联合排序与人工建议

### Task 20：冻结 Alpha158 信号与联合排序合同（已完成）

**Description:** 定义独立 Alpha158 分数、可靠性、联合权重和回退语义。

**Acceptance criteria:**
- [x] Alpha158 输出包含模型版本、行情时点、原始分、分位和可靠性。
- [x] Alpha158 不可用时联合分逐项等于 V3 原始分。
- [x] TopN 只用于展示或研究，不得成为生产候选硬过滤器。

**Verification:** JS/Python 合同金样与非法输入测试。

### Task 21：实现 Alpha158 模型训练、注册表和批量推理（已完成）

**Description:** 从完整主板日线构造 158 维因子，训练并原子发布独立排序模型。

**Acceptance criteria:**
- [x] 训练、校准、选择、确认按标签区间隔离。
- [x] 推理使用不少于60根的点时OHLCVA，除权口径一致。
- [x] 模型缺失、过期或Rank IC不为正时返回不可用，不阻断V3。

**Verification:** Python 单测、同一历史时点前缀不变测试、模型包哈希测试。

### Task 22：接入全市场多打法召回和 V3 联合排序（已完成）

**Description:** 批量获取 Alpha158 连续分数并与 V3 排序分融合。

**Acceptance criteria:**
- [x] 每只候选保留 V3 分、Alpha158 分、权重和联合分。
- [x] 联合权重由质量门禁冻结，线上不得自行调参。
- [x] Alpha158 不改变价格合同、手数、成交概率或风险动作。

**Verification:** 候选排序、缺失回退、过期拒绝和同分稳定性测试。

### Task 23：统一唯一人工操作建议（已完成）

**Description:** 将组合仲裁结果收敛为同一决策版本下唯一可核对动作。

**Acceptance criteria:**
- [x] 建议包含动作、整数手数、价格区间、有效期、止损、目标和失效条件。
- [x] `USER_CONFIRMED`不改变现金或持仓。
- [x] 只有真实成交推进执行状态并形成归因记录。

**Verification:** 执行计划、账户账本和展示契约测试。

### Task 24：回填联合特征并执行同源账户对比（当前研究样本已完成）

**Description:** 将 Alpha158 扩展时序样本外分数关联到历史候选，比较现役 V3
与联合模型。

**Acceptance criteria:**
- [x] 不使用全量拟合分数回填历史。
- [x] 5bps、10bps、执行延迟和账户约束使用同一回放。
- [x] 披露资金利用率、费用、回撤、收益集中度和逐段结果。

**Verification:** `npm run harness:execution`、账户分币审计和版本化报告。

### Task 25：联合模型每日训练与选择性发布（已完成）

**Description:** 日训同时维护 Alpha158 信号和联合排序挑战者。

**Acceptance criteria:**
- [x] 仅使用沪深主板完整数据。
- [x] 挑战者与现役模型在共同新增留出集比较。
- [x] 未同时改善费后账户收益和风险指标时保持现役 V3。

**Verification:** GitHub Actions 合同测试、手动工作流和线上只读版本检查。
