...
### Task 20：冻结 Alpha158 信号与联合排序合同

**Description:** 定义独立 Alpha158 分数、可靠性、联合权重和回退语义。

**Acceptance criteria:**
- [ ] Alpha158 输出包含模型版本、行情时点、原始分、分位和可靠性。
- [ ] Alpha158 不可用时联合分逐项等于 V3 原始分。
- [ ] TopN 只用于展示或研究，不得成为生产候选硬过滤器。

**Verification:** JS/Python 合同金样与非法输入测试。

### Task 21：实现 Alpha158 模型训练、注册表和批量推理

**Description:** 从完整主板日线构造 158 维因子，训练并原子发布独立排序模型。

**Acceptance criteria:**
- [ ] 训练、校准、选择、确认按标签区间隔离。
- [ ] 推理只接受不少于60根的点时OHLCVA，除权口径一致。
- [ ] 模型缺失、过期或Rank IC不为正时返回不可用，不阻断V3。

**Verification:** Python 单测、同一历史时点前缀不变测试、模型包哈希测试。

### Task 22：接入全市场多打法召回和 V3 联合排序

**Description:** 批量获取 Alpha158 连续分数并与 V3 排序分融合。

**Acceptance criteria:**
- [ ] 每只候选保留 V3 分、Alpha158 分、权重和联合分。
- [ ] 联合权重由选择段冻结，线上不得自行调参。
- [ ] Alpha158 不改变价格合同、手数、成交概率或风险动作。
...
**Acceptance criteria:**
- [ ] 建议包含动作、整数手数、价格区间、有效期、止损、目标和失效条件。
- [ ] `USER_CONFIRMED`不改变现金或持仓。
- [ ] 只有真实成交推进执行状态并形成归因记录。

**Verification:** 执行计划、账户账本和展示契约测试。

### Task 24：回填联合特征并执行同源账户对比
...
**Acceptance criteria:**
- [ ] 不使用全量拟合分数回填历史。
- [ ] 5bps、10bps、执行延迟和账户约束使用同一回放。
- [ ] 披露资金利用率、费用、回撤、收益集中度和逐段结果。

**Verification:** `npm run harness:execution`、账户分币审计和版本化报告。

### Task 25：联合模型每日训练与选择性发布
