...
7. 普通 `REDUCE/EXIT` 仍先进入约 10 分钟退出复核；复核可改回 `HOLD`。
...
- Node：`2297/2297`
- Harness：`53/53`
- 连续市场：240 步，28 条决策，`HOLD 17 / REDUCE 1 / WATCH 10`，
  10 笔订单，8 笔流水，0 问题，0 云端写入
- 线上测试账号：V2 完整输出持有、部分减仓、退出的独立价值分解
- Vercel、FC 自定义域名、报价端点与 AI ping：HTTP 200
