...
const fields: Record<string, string> = {
  ...
  executionLink: "买入成交来源", openingLink: "期初批次来源",
  acquisitionBeforeSnapshot: "期初取得日期", executionAfterOpening: "成交与期初顺序",
...
export function Reconciliation({ accountId, version }: { accountId: string; version: number }) {
  ...
  return <section className="ledger-section">
    ...
    <p className="source-note">从资金、期初持仓与成交记录独立重算，检查现金、持仓批次和费用。核对通过表示平台内部一致；券商交割单仍需另行比对。</p>
    ...
    {report && <div role="status">
      ...
      {!!report.openingLotCount && <p className="source-note">包含 {report.openingLotCount} 个期初取得批次。</p>}
