...
面向 A 股短线、做 T 和持仓纪律执行的个人交易工作台。系统把公开行情、模块化量化决策、价格预警、交易记录和复盘整合成一条可审计链路。
...
- 持仓与自选按核心概念或行业筛选。
- 模块化决策引擎统一生成动作、价格、手数和费后期望；LLM 只提供不改变决策的白话解释。
- 系统决策可加入人工执行队列，按到价、用户确认、部分成交和完成状态闭环记录。
...
- 持仓组合分析只汇总当前有效系统决策，生成目标仓位、个股执行单和失效条件。
- 组合白话解释失败不影响系统执行单，结果和历史记录继续持久化。
- 盯盘预警采用“触价后观察约 10 分钟 -> 复核动作价值 -> 统一仲裁”两段式确认；普通退出也走同一观察窗口，账本硬止损除外。
...
- 系统决策、执行计划、成交归因、预警、持仓分析和复盘支持跨设备增量同步。
...
| 量化服务 | 独立 Python 服务 | 多任务动作价值、36 因子日线辅助预测和模型元数据 |
| 存储 | 阿里云 OSS | 账号快照、配置、任务、报告、模型和难样本 |
| 定时 | FC Timer + GitHub Actions | 盯盘、复盘、任务恢复、决策样本结算和每日训练 |
...
| 角色 | 用途 | 默认模型 |
|---|---|---|
| `explain` | 系统单股和组合白话解释（固定两个端点） | `DeepSeek-V4-Pro` |
...
决策任务保留 `advisor` / `review` 两条历史 lane 名称，以兼容已持久化运行态；它们不是 LLM 角色。单股评估与到价复核都直接运行模块化决策引擎，不占用解释端点。旧 `advisor`、`review`、`portfolio`、`agent`、`judge` 配置只在首次读取时迁移到四角色，保存后不再写回。

到价复核采用事件驱动快路径：页面在线时一旦实时价命中观察位，会立即通过账号鉴权接口把预警切为观察中，云端 Timer 负责离线兜底。系统持续采集约 10 分钟触价后路径，窗口结束后才排入唯一一次紧急复核任务，不占用 Worker 原地等待。普通模型减仓或退出不要求价格再次穿越旧参考价；随后无论反弹还是下跌，都使用最新价格、分时、资金和复核动作价值结果重评。转为正期望时撤销旧退出，仍为负期望时才按最新价格授权退出。复核模型未通过发布门禁时失败关闭，账本硬止损不等待该窗口。
...
- 同股重复提交按幂等键合并；每轮系统决策只使用一次完整证据快照，真实交易变化最多即时重算一次。
- 主决策评估默认并发 4 路（兼容环境变量 `V3_DECISION_CONCURRENCY`，硬上限 8），与两个 `explain` 端点无关；到价复核另用独立容量。提交响应丢失时最多核对 30 秒，权威任务出现后立即切换为真实进度。
- 运行期间仅真实持仓、成交、现金或执行计划变化会使旧结论失效；题材、行业和同步时间戳更新不会制造假重排。
- 主力/小单资金说明由服务端写入真实的当日净额、完整五日双序列与五日合计；历史不足五日时只显示实际天数。
- 系统决策不等待 LLM、新闻、搜索或策略日报；这些信息只进入后置解释和研究层。
- 主力/小单资金并行比较多个历史镜像并选择交易日最完整的数据；不足5日时明确标注实际天数，禁止伪称“最近5日序列”。
- 相同用户请求、触发事件和已被当前证据覆盖的复核均按幂等键去重。
- 只有完整系统结果成功落盘后才替换旧决策；白话解释失败不得撤回或覆盖该结果。
...
组合分析由服务端读取账户内每只股票的当前有效系统决策，按现金、费用、T+1
...
- 使用 136 维 `opportunity-score-feature.v6` 特征合同；新增平台突破、
  缩量回踩、量价齐升、长下影修复和低波动趋势形态特征。
...
- 只作为系统决策的辅助证据，不得覆盖动作、价格、手数、排序或风险核定。
...
api/                    FC 后端路由与共享模块
  ...
  _confirm.js           两段式交易确认
  portfolio_analysis.js 持仓组合决策汇总与后置解释
