...
| `review` | 委员会、定时与 Judge 失效复核（固定两个端点） | `DeepSeek-V4-Pro` |
...
配置使用 `roleEndpoints` 按角色严格隔离。`advisor` 和 `review` 各使用两个端点按最少在途路由，其余角色各使用一个独立端点；任何角色都不会回退到其他角色。侧边栏对话统一使用 `agent`。普通军师生成强制关闭深度思考，选择深度生成时强制开启。端点连续失败后会熔断、冷却并半开恢复。
