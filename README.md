...
- **AI 选股(全市场 + 量化 + 大模型融合)**:扫描全市场并过滤 ST/退市/停牌/低流动性/追高标的,再做确定性横截面评分、量化复排和 LLM 交易价值研判。候选按**可执行 / 等待触发 / 观察**分层,给出入选数据、买入区、目标、止损、确认信号与失效条件。
- **不再用空名单代替判断**:没有立即买点时仍保留 1~3 只相对最优条件候选;LLM 或量化暂时不可用时,自动回退到确定性排名与价格锚点,避免“市场没有股票”的无效结果。
- **时段感知**:交易时段生成实时候选;盘前/收盘后也可生成**下一交易日开盘观察池**。结果同步保存到账号 OSS,跨设备与刷新后仍可查看。
...
### 💰 账户 · 交易
- **动态账户全景**:当前总资产始终按“可用现金 + 实时持仓市值”计算;买入、卖出、做 T 自动更新现金并计入手续费,未结算做 T 净头寸也进入市值。用户设置的是**投入本金 / 盈亏基准**,系统据此展示累计盈亏、仓位%、持仓分布和单票超配预警。
...
- **AI 操作建议卡(核心)**:
  ...
  - **分钟 Transformer V2**:可按账号手动切换并按需启停 EAS 服务,输出下一交易日三重障碍概率、概率优势、动量/波动/量能/位置等上下文,以及锚点、支撑、压力、参考买入区、止盈与止损价格。
...
- 服务端**并发池 + 持久任务表**(生命周期/断点续跑/防重/取消),并发上限绑定 advisor 端点数;生成阶段、数据源、量化结论、模型端点与中文推理文本持续回灌,浏览器切后台不再中断。
