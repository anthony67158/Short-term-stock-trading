...
- 今日精选候选池保留涨停、主力抢筹和涨速等客观盘面信号，不再重复维护独立 AI 选股模块。
...
- 持仓与自选按核心概念或行业筛选。
- AI 操作建议支持快速生成、显式深度研判、事件监控和跨页面后台生成。
- 建议可加入人工执行队列，按到价、用户确认、部分成交和完成状态闭环记录。
...
- AI 建议、执行计划、成交归因、预警、持仓分析和复盘支持跨设备增量同步。
...
| `review` | 委员会、定时与 Judge 失效复核（固定四个端点） | `DeepSeek-V4-Pro` |
...
配置使用 `roleEndpoints` 按角色严格隔离。`advisor` 使用两个端点，`review` 使用四个端点，其余角色各使用一个独立端点；同角色先轮询采样，再结合最少在途与近期响应时长路由，任何角色都不会回退到其他角色。复核调度最多允许普通定时任务占用两路，至少为到价与 Judge 紧急复核预留两路。侧边栏对话统一使用 `agent`。快速生成与复核显式下发 `reasoning_effort=none`，选择深度生成时才开启中等推理。端点连续失败后会熔断、冷却并半开恢复。

操作建议任务按角色分为两条独立 lane：单股手动生成与一次性生成进入 `advisor`，定时复核和 Judge 失效复核进入 `review`。两条 lane 可并行，同一股票也不会互相覆盖任务状态；深度任务进入委员会阶段后会释放 advisor 容量，由 review 端点继续复核。主批次进度只统计 advisor，后台复核只在对应股票上显示，避免已完成批次被误显示为重新生成。

到价复核采用低延迟快路径：页面在线时一旦实时价命中观察位，会立即通过账号鉴权接口把预警切为复核中并排入紧急任务，云端 Timer 负责离线兜底。Worker 最多每秒发现一次新紧急任务，重新采集实时价格、分时、主力/散户资金、日线技术、大盘与板块资金；新闻、龙虎榜、量化和日报复用原建议，避免重复慢采集。模型预算最多 45 秒且完整交给唯一一次模型调用，响应开始后不换端点、不重跑终稿；失败时直接形成终局降级结论。每次提醒的页面横幅、浏览器通知、声音和预警中心记录共用同一份股票、触发条件与操作文案。
...
- 普通生成、深度研判和复核每个任务只做一次完整生成；同股快速/深度提交共用在途锁，重复点击不会再次入队。底层仅允许在尚未收到响应头时切换同角色端点。快速生成只等待价格决策必需证据，总预算 55 秒；深度研判再补新闻、搜索、龙虎榜和策略背景。
- 生成期间仅真实持仓、成交、现金或执行计划变化会使旧结论失效；题材、行业、量化分和同步时间戳更新不会把运行中的深度任务重新排队。真实交易变化最多即时重算一次。
- 主力/小单资金说明由服务端写入真实的当日净额、完整五日双序列与五日合计，模型不能自行抄写数值；历史不足五日时只显示实际天数。
- 快速建议不等待策略日报；深度研判的日报补充失败时降级该证据，个股主流程继续。
- 主力/小单资金并行比较多个历史镜像并选择交易日最完整的数据；不足5日时明确标注实际天数，禁止伪称“最近5日序列”。
- 相同用户请求、Judge 事件和已被当前证据覆盖的触发均按幂等键去重。
- Judge 确认只推进确定性执行状态；只有计划失效或软证据冲突才重新调用军师。
- 生成过程按准备上下文、多源证据、量化校验、生成结论展示；旧结果会明确标记，只有完整新结果落盘后才替换。
