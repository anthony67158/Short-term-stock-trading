...
配置使用 `roleEndpoints` 按角色严格隔离。`advisor` 使用两个端点，`review` 使用四个端点，其余角色各使用一个独立端点；同角色按最少在途路由，任何角色都不会回退到其他角色。复核调度最多允许普通定时任务占用两路，至少为到价与 Judge 紧急复核预留两路。侧边栏对话统一使用 `agent`。普通军师生成强制关闭深度思考，选择深度生成时强制开启。端点连续失败后会熔断、冷却并半开恢复。
...
到价复核采用低延迟快路径：页面在线时一旦实时价命中观察位，会立即通过账号鉴权接口把预警切为复核中并排入紧急任务，云端 Timer 负责离线兜底。Worker 最多每秒发现一次新紧急任务，重新采集实时价格、分时、主力/散户资金、日线技术、大盘与板块资金；新闻、龙虎榜、量化和日报复用原建议，避免重复慢采集。模型预算最多 45 秒、单次尝试，2 分钟硬截止负责异常兜底。每次提醒的页面横幅、浏览器通知、声音和预警中心记录共用同一份股票、触发条件与操作文案。
...
- 普通生成、深度研判和复核每个任务只做一次完整生成；同股快速/深度提交共用在途锁，重复点击不会再次入队。底层仅允许在尚未收到响应头时切换同角色端点。
