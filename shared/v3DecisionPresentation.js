...
export function v3DecisionPresentation({
  ...
  const isV3 = source?.engine === 'V3'
  const plan = isV3 ? advice?.decisionPlan : null
  ...
  const active = isV3 && Number.isFinite(expiresAt) && !expired && !completed
  ...
  if (hardStop) {
    ...
    headline = '等待可卖时段退出'
    reason = 'V3剩余路径费后价值不为正，但今日仓位受T+1锁定。'
    timing = '下一可卖交易时段'
  } else if (priceMoved) {
    headline = held ? '暂不加仓' : '暂不买入'
    ...
    reason = held ? 'V3剩余路径价值仍为正，本次不加仓、不减仓。'
      : '当前没有费后价值为正的买入路径。'
  }
  return {
